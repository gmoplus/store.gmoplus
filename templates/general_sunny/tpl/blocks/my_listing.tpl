	...
	<div class="nav">
		...
		<div class="navigation">
			<ul>
				...
				{* GMO Plus TASK 8 & 9: İlan Yenileme Butonu - Refresh Listing Button *}
				{if $listing.Status == 'active'}
					<li class="nav-icon">
						<a class="refresh-listing" 
						   ...
						   data-listing-type="{if $listing_type.Key}{$listing_type.Key}{else}general{/if}"
						   title="{$lang.refresh_listing}">
							<span>{$lang.refresh_listing}</span>&nbsp;
						</a>
					</li>
				{/if}
		...
		<div class="stat">
			<ul>
				...
				{* GMO Plus TASK 8 & 9: Yenileme Bilgileri - Refresh Information *}
				{if $listing.Status == 'active' && $config.listing_refresh_enabled}
					<li class="refresh-info" id="refresh_info_{$listing.ID}">
						<span class="name">{$lang.refresh_remaining}</span>
						<span class="refresh-remaining-count" data-listing-id="{$listing.ID}" data-listing-type="{if $listing_type.Key}{$listing_type.Key}{else}general{/if}">
							<i class="loading-icon" style="display:none;">⏳</i>
