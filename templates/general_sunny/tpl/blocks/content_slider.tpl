...
<div id="homeCarousel" class="carousel slide carousel-fade{if !(!$is_form_arranged && is_array($search_forms) && $search_forms|@count > 1)} no-search-tabs{/if}" data-ride="carousel">
    ...
    <div class="carousel-inner h-100">
        {foreach from=$home_slides item='slide' name='slides'}
            <div class="carousel-item{if $smarty.foreach.slides.first} active{/if}{if !trim($slide.title)} carousel-item_no-text{/if}">
                ...
                <div class="carousel-caption">
                    ...
                    {if $slide.URL}
                    <a class="mt-md-4 button default" href="{$slide.URL}">{$lang.view_details}</a>
                    {/if}
                </div>
            </div>
        {/foreach}
