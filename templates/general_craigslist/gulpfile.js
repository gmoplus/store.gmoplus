...
var coreSassDirs = [
    ...
    '!' + path.resolve(coreSrc, '**', 'search_map.sass'),
];
...
var fixDest = function() {
    return rename(function(path) {
        if (/css\/|\\s(a|c)ss/.test(path.dirname)) {
            path.dirname = 'css';
        }
    });
}
...
var migration = function() {
    return map(function(file, callback) {
        ...
        if (file_name == 'style.css' || file_name == 'style-rtl.css') {
            var postfix = file_name == 'style-rtl.css' ? '-rtl' : ''; 
            var contents = file.contents.toString();
            var original = fs.readFileSync(src + '/css/original-style' + postfix + '.css').toString();
