...
<div class="grid_navbar listings-area row">
    <div class="col-6">
        <div class="switcher">{strip}
            ...
            <div class="current d-sm-none">{$grid_mode|@capitalize}</div>
            <div class="buttons">
                <div data-type="list" class="list{if $grid_mode == 'list'} active{/if}" title="{$lang.list_view}">{$lang.list_view}</div>
                {if $listing_type && !$listing_type.Photo}{else}
                    <div data-type="grid" class="grid{if $grid_mode == 'grid'} active{/if}" title="{$lang.gallery_view}">{$lang.gallery_view}</div>
                {/if}
                {if $config.map_module}
                <div data-type="map" class="map{if $grid_mode == 'map'} active{/if}" title="{$lang.map}">{$lang.map}</div>
                {/if}
            {/strip}</div>
        </div>
    </div>
				...
				{if isset($field_item.Details_page) && $field_item.Details_page == '0' || $field_item.Type == 'checkbox'}{continue}{/if}
                    ...
                    {if $field_item.Type|in_array:$sf_types}
                        {foreach from=$sort_types key='st_key' item='st'}
                            <li{if $sort_by == $sort_key && $sort_type == $st_key} class="active"{/if}><a rel="nofollow" title="{$lang.sort_listings_by} {$field_item.name} ({$lang[$st]})" href="{if $config.mod_rewrite}?{else}index.php?{$pageInfo.query_string}&{/if}sort_by={$sort_key}&sort_type={$st_key}">{$field_item.name} ({$lang[$st]})</a></li>
                        {/foreach}
                    {else}
