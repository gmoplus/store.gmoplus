...
<div class="point1{if $pageInfo.Key == 'home'}{if !$home_slides} no-slides{/if} header-tabs{if !$is_form_arranged && is_array($search_forms) && $search_forms|@count > 1} header-tabs__tabs-exists{/if}{/if}">
<!-- tabs -->

{if is_array($search_forms) && $search_forms|@count > 1 && !$is_form_arranged}
            ...
            <li id="tab_{$sf_key}" class="{if $smarty.foreach.stabsF.first}active{/if}">
                <a href="#{$sf_key}" data-target="{$sf_key}"
                    ...
                    {if $pageInfo.Key == 'home' && $listing_types[$search_form.listing_type].Menu && $listing_types[$search_form.listing_type].Menu_icon}
                        <span class="tab-icon">
                            {fetch file=$smarty.const.RL_LIBS|cat:'icons/svg-line-set/'|cat:$listing_types[$search_form.listing_type].Menu_icon}
                        </span>
                    {/if}
                    {$search_form.name}
                </a>
