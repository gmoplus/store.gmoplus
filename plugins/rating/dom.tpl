...
{if $listing_data.lr_rating_votes}
...
{else}
    {assign var='average_rating' value=0}
{/if}
...
<ul class="listing_rating_ul{if !$rating_denied && (($config.rating_prevent_visitor && $isLogin) || !$config.rating_prevent_visitor) && (!$config.rating_prevent_owner || ($config.rating_prevent_owner && $listing_data.Account_ID != $account_info.ID))} listing_rating_available{/if}">
