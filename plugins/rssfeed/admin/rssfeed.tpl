...
<!-- navigation bar -->
<div id="nav_bar">
    <a href="{$rlBaseC}action=add" class="button_bar"><span class="left"></span><span
                class="center_add">{$lang.add_block}</span><span class="right"></span></a>
    <a href="{$rlBaseC}" class="button_bar"><span class="left"></span><span
                class="center_list">{$lang.items_list}</span><span class="right"></span></a>
</div>
<!-- navigation bar end -->
...
{if $smarty.get.action == 'edit' || $smarty.get.action == 'add'}
    ...
    <form action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;id={$smarty.get.id}{/if}"
          method="post">
        ...
        <table class="form">
            ...
            <tr>
                ...
                <td class="field">
                    ...
                    {foreach from=$allLangs item='language' name='langF'}
                        ...
                        <input type="text" name="name[{$language.Code}]" value="{$sPost.name[$language.Code]}"
                               maxlength="350"/>
                        {if $allLangs|@count > 1}
            ...
            <tr>
                ...
                <td class="field" id="rssfeed_url">
                    <input type="text" name="url" value="{$sPost.url}" class="w350"/> <input style="margin-top: 0;"
                                                                                             type="button"
                                                                                             value="{$lang.rssfeed_validate}"/>
            ...
            <tr>
                ...
                <td class="field">
                    <input type="text" name="article_num" value="{$sPost.article_num}" maxlength="5" class="w60"
            ...
            <tr>
                ...
                <td class="field">
                    <input type="text" name="update_delay"
                           value="{if $sPost.update_delay}{$sPost.update_delay}{else}12{/if}" maxlength="5" class="w60"
                           style="text-align: center;"/> <span>{$lang.rssfeed_hours}</span>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="field">
                    <select name="status">
                        <option value="active"
                                {if $sPost.status == 'active'}selected="selected"{/if}>{$lang.active}</option>
                        <option value="approval"
                                {if $sPost.status == 'approval'}selected="selected"{/if}>{$lang.approval}</option>
                    </select>
                </td>
            </tr>
            <tr>
