...
{if $smarty.get.action == 'edit'}
	...
	<form action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;key={$smarty.get.key}{/if}" method="post">
		...
		<tr>
			<td class="name">{$lang.iflynax_type_icon}</td>
			<td class="field">
					...
					<div style="display: inline-block;*display: inline;*zoom: 1;text-align: center;">
						<label>
							<div style="background: #43494a;width: 42px;height: 34px;border-radius: 3px;padding-top: 8px;margin-bottom: 4px;">
								<img width="26" height="26" src="{$smarty.const.RL_PLUGINS_URL}iFlynaxConnect/static/icon_{$icon}.png" />
							</div>
...
{else}
	...
	$(document).ready(function(){
		...
		listingTypesGrid = new gridObj({
			key: 'listingTypes',
			id: 'grid',
			ajaxUrl: rlPlugins + 'iFlynaxConnect/admin/iflynax_listing_types.inc.php?q=ext',
			defaultSortField: 'name',
			title: lang['ext_listing_types_manager'],
            reloadOnUpdate: true,
			fields: [
				{name: 'name', mapping: 'name', type: 'string'},
				{name: 'iFlynax_position', mapping: 'iFlynax_position', type: 'int'},
				{name: 'Status', mapping: 'iFlynax_status'},
