			...
			<td class="field">
				<div class="inner_margin">
					...
					{if $configItem.Type == 'text'}
						<input class="text {if $configItem.Data_type == 'int'}numeric{/if}" type="text" name="config[{$configItem.Key}][value]" value="{$configItem.Default}" style="float: left;" />
					...
					{elseif $configItem.Type == 'select'}
						<select style="width: 204px;" name="config[{$configItem.Key}][value]" 
							{foreach from=$configItem.Values item='sValue' name='sForeach'}
								...
								>
									{if is_array($sValue) && !$sValue.required}<option value="">{$lang.select}</option>{/if}
								{/if}
                    ...
                    <span class="settings_desc" style="display:block;line-height:20px;padding:0px 0 10px 20px;overflow:hidden;">
					{if $configItem.Key == 'iflynax_promote_app'}
						{$lang.iflynax_promote_app}
					{elseif $configItem.Key == 'iflynax_app_store_id'}
						{$lang.iflynax_app_store_id}
					{elseif $configItem.Key == 'iflynax_affiliate_data'}
						{$lang.iflynax_affiliate_data}
                    {elseif $configItem.Key == 'iflynax_inapp_module'}
						{$lang.iflynax_inapp_module}
                    {elseif $configItem.Key == 'iflynax_inapp_sandbox'}
						{$lang.iflynax_inapp_sandbox}
					{/if}
                    </span>
				</div>
			</td>
