<!-- iflynax languages tpl -->

{if $smarty.get.action == 'edit'}

	<!-- navigation bar -->
	<div id="nav_bar">
		<a href="{$rlBase}index.php?controller={$smarty.get.controller}" class="button_bar">{strip}
			<span class="left"></span><span class="center_list">{$lang.languages_list}</span><span class="right"></span>
		{/strip}</a>
	</div>
	<!-- navigation bar end -->
			...
			<div style="padding: 7px 0">
				{if $compare_lang1.diff}
					{if $smarty.post.compare_mode == 'phrases'}
						...
						<input style="margin-top: 5px;" id="copy_button_1" onclick="xajax_copyPhrases(1, 2, '{$langs_info.$code_2.name}');$('#loading_1').fadeIn('normal');" type="button" value="{$lang.compare_copy_phrases|replace:$replace_lang1:$langs_info.$code_1.name|replace:$replace_lang2:$langs_info.$code_2.name}" />
			...
			<div style="padding: 20px 0 7px 0">
				{if $compare_lang2.diff}
					{if $smarty.post.compare_mode == 'phrases'}
						...
						<input style="margin-top: 5px;" id="copy_button_2" onclick="xajax_copyPhrases(2, 1, '{$langs_info.$code_1.name}');$('#loading_2').fadeIn('normal');" type="button" value="{$lang.compare_copy_phrases|replace:$replace_lang1:$langs_info.$code_2.name|replace:$replace_lang2:$langs_info.$code_1.name}" />
		...
		{if $compare_lang1.diff}
			...
			$(document).ready(function(){
				compareGrid1 = new gridObj({
					key: 'compare1',
					id: 'compare_grid1',
					ajaxUrl: rlPlugins + 'iFlynaxConnect/admin/iflynax_languages.inc.php?q=compare&grid=1&compare_mode='+compare_mode,
		...
		{if $compare_lang2.diff}
			...
			$(document).ready(function(){
				...
				compareGrid2 = new gridObj({
					key: 'compare2',
					id: 'compare_grid2',
					ajaxUrl: rlPlugins + 'iFlynaxConnect/admin/iflynax_languages.inc.php?q=compare&grid=2&compare_mode='+compare_mode,
					defaultSortField: 'Value',
...
<div id="nav_bar">
	{if $aRights.$cKey.add}
		<a href="javascript:void(0)" onclick="show('import', '#action_blocks div');" class="button_bar">{strip}
			<span class="left"></span><span class="center_import">{$lang.import}</span><span class="right"></span>
		{/strip}</a>
	{/if}
	
	{if $aRights.$cKey.edit}
		<a href="javascript:void(0)" onclick="show('compare', '#action_blocks div');" class="button_bar">{strip}
			<span class="left"></span><span class="center_compare">{$lang.compare}</span><span class="right"></span>
		{/strip}</a>
	{/if}
	
	{if $aRights.$cKey.add}
		<a href="javascript:void(0)" onclick="show('lang_add_container', '#action_blocks div');" class="button_bar">{strip}
			<span class="left"></span><span class="center_add">{$lang.add_language}</span><span class="right"></span>
		{/strip}</a>
	{/if}

    {if isset($smarty.get.dev)} {* Hidden ability for developers *}
        <a href="javascript:void(0)" onclick="show('customphrase_add_container', '#action_blocks div');" class="button_bar">{strip}
            <span class="left"></span><span class="center_add">Add a new custom Phrase</span><span class="right"></span>
        {/strip}</a>
    {/if}
</div>
<!-- navigation bar end -->
...
<div id="action_blocks">
    ...
    {if isset($smarty.get.dev)} {* Hidden ability for developers *}
    <div id="customphrase_add_container" class="hide">
        {include file='blocks'|cat:$smarty.const.RL_DS|cat:'m_block_start.tpl' block_caption="Add a new custom Phrase"}
        <form id="add_phrase_form" onsubmit="return false;">
        <table class="form">
            <tr>
                <td class="name"><span class="red">*</span>{$lang.key}</td>
                <td class="field">
                    <input type="text" id="phrase_key" style="width: 300px;" maxlength="255" />
                </td>
            </tr>
            <tr>
                <td class="name">{$lang.value}</td>
                <td class="field">
                    <input type="text" id="phrase_value" style="width: 300px;" />
                </td>
            </tr>

            {if $langCount > 1}
            <tr>
                <td class="name">Add to</td>
                <td class="field">
                    <select id="phrase_code">
            ...
            {else}
                <input type="hidden" id="phrase_code" value="all" />
            {/if}
            
            <tr>
                <td></td>
                <td class="field">
                    <input onclick="addPhrase()" type="button" value="{$lang.add}" />
                    <span class="loader" id="lang_add_load"></span>
                    <a class="cancel" href="javascript:void(0)" onclick="show('customphrase_add_container')">
                        {$lang.cancel}
                    </a>
                </td>
            </tr>
    ...
    {literal}
        function addPhrase() {
            var phrase_key = $('input#phrase_key').val().trim();
            $('input#phrase_key').val(phrase_key);

            if (phrase_key == '') {
                return alert('The key is required')
            }

            $.ajax({
                method: 'POST',
                url: rlPlugins + 'iFlynaxConnect/admin/iflynax_languages.inc.php?q=add_phrase',
                data: {
                    key: phrase_key,
                    value: $('input#phrase_value').val(),
                    code: $('#phrase_code').val()
                }
            }).done(function() {
	...
	<div id="import" class="hide">
		...
		<form action="{$rlBase}index.php?controller={$smarty.get.controller}&amp;import" method="post" enctype="multipart/form-data">
			...
			<table class="form">
			<tr>
				<td class="name"><span class="red">*</span>{$lang.iflynax_xml_dump}</td>
				<td class="field">
	...
	<div id="compare" class="hide">
		...
		<table class="form">
		<tr>
			<td class="name" style="width: 250px;"><span class="red">*</span>{$lang.compare}</td>
			<td class="field">
				<select name="lang_1" id="lang_1">
				<option value="">{$lang.select}</option>
					{foreach from=$allLangs item='lang_list'}
...
$(document).ready(function(){
	...
	languagesGrid = new gridObj({
		key: 'languages',
		id: 'grid',
		ajaxUrl: rlPlugins + 'iFlynaxConnect/admin/iflynax_languages.inc.php?q=ext_list',
...
$(document).ready(function(){
	...
	phrasesGrid = new gridObj({
		key: 'phrases',
		id: 'phrases',
		ajaxUrl: rlPlugins + 'iFlynaxConnect/admin/iflynax_languages.inc.php?q=ext',
		defaultSortField: 'Value',
