...
{elseif $config.tc_box_type == 'simple'}
    <div id="cloud">
        {foreach from=$tag_cloud item='tag'}
            <a {if $config.tc_tag_new_page}target="_blank"{/if} href="{$smarty.const.SEO_BASE}{if $config.mod_rewrite}{$pages.tags}/{$tag.Path}{if $config.tc_urls_postfix}.html{else}/{/if}{else}?page={$pages.tags}&tag={$tag.Path}{/if}" style="font-size:{$tag.Size}px;">{$tag.Tag|@stripcslashes}</a>
        {/foreach}
    </div>
{elseif $config.tc_box_type == 'gradient'}
    {addJS file=$smarty.const.RL_PLUGINS_URL|cat:'tag_cloud/static/jquery.tagcloud.js'}
...
{elseif $config.tc_box_type == 'circle'}
    {addCSS file=$smarty.const.RL_PLUGINS_URL|cat:'tag_cloud/static/jqcloud.css'}
    {addJS file=$smarty.const.RL_PLUGINS_URL|cat:'tag_cloud/static/jqcloud.js'}
    ...
    <script class="fl-js-dynamic">
        var word_list = [
            {foreach from=$tag_cloud item='tag' name='tagsLoop'}
                ...
                {literal}{
                        text  : "{/literal}{$tag.Tag}",
                        weight: {if $tag.Size != 'NAN'}{$tag.Size}{else}11{/if},
                        count : {$tag.Count}{literal},
