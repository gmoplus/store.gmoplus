{if $field.Condition && is_array($multi_format_keys) && $field.Condition|in_array:$multi_format_keys}
    ...
    <script>
        ...
        {if $smarty.post.$mf_form_prefix[$field_key] || $smarty.request.$mf_form_prefix[$field_key]}
            mfFieldVals['{$head_field_key}']['{$field_key}'] = '{if $smarty.post.$mf_form_prefix[$field_key]}{$smarty.post.$mf_form_prefix[$field_key]}{else}{$smarty.request.$mf_form_prefix[$field_key]}{/if}';
            {assign var='mf_data_source' value='post'}
        {elseif $smarty.post.$field_key || $smarty.request.$field_key}
            mfFieldVals['{$head_field_key}']['{$field_key}'] = '{if $smarty.post.$field_key}{$smarty.post.$field_key}{else}{$smarty.request.$field_key}{/if}';
            {assign var='mf_data_source' value='post'}
