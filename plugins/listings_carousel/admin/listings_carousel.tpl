...
{assign var='sPost' value=$smarty.post}
    ...
    {include file='blocks'|cat:$smarty.const.RL_DS|cat:'m_block_start.tpl'}
        <form action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;id={$smarty.get.id}{/if}" method="post" enctype="multipart/form-data">
            ...
            <tr style="display: none;">
                ...
                <td>
                    <select id="direction" name="direction">
                        <option value="horizontal" selected="selected">{$lang.listings_carousel_horizontal}</option>
                    ...
                    <fieldset class="light">
                        ...
                        <div id="type">
                            <table id="list_box">
                                <tr>
                                    ...
                                    <div style="padding: 2px 8px;">
                                        ...
                                        <input {if $item.disabled && $sPost.box && !$item.ID|in_array:$sPost.box}accesskey="{$item.disabled}" disabled="disabled" checked="checked"{/if}
                                               class="checkbox"
                                               {if $sPost.box && $item.ID|in_array:$sPost.box}checked="checked"{/if}
                                               id="type_{$item.ID}"
                                               type="checkbox"
                                               name="box[{$item.ID}]"
                                               value="{$item.ID}"
                                        /> <label class="cLabel" for="type_{$item.ID}">{phrase key=$name db_check='true'}</label>
                                    </div>
                                    </td>
                            ...
                            <div class="grey_area" style="margin: 0 0 5px;">
                                <span>
                                    <span onclick="$('#list_box input:not(:disabled)').prop('checked', true);" class="green_10">{$lang.check_all}</span>
                                    <span class="divider"> | </span>
                                    <span onclick="$('#list_box input:not(:disabled)').prop('checked', false);" class="green_10">{$lang.uncheck_all}</span>
                                </span>
                            </div>
                        </div>
                    </fieldset>
            ...
            <tr>
                <td class="name">{$lang.listings_carousel_delay}</td>
                <td class="field">
