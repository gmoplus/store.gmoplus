...
<div id="nav_bar">
    ...
    {if $aRights.$cKey.add && !$smarty.get.action}
        <a href="{$rlBaseC}action=add" class="button_bar"><span class="left"></span><span class="center-add">{$lang.add_admin}</span><span class="right"></span></a>
    {/if}
        ...
        <div id="super_area" {if $sPost.type == 'super' || !isset($sPost.type)}class="hide"{/if}>
            ...
            {foreach from=$mMenuItems item='nParent'}
                <fieldset style="margin: 5px 0;">
                    <legend id="legend_parent_{$nParent.ID}" class="up"><span onclick="fieldset_action('parent_{$nParent.ID}');">{$nParent.name}</span> (<span class="purple_12_cursor" onclick="$('#parent_{$nParent.ID} input').prop('checked', true);">{$lang.check_all}</span><label>|</label><span class="purple_12_cursor" onclick="$('#parent_{$nParent.ID} input').prop('checked', false);">{$lang.uncheck_all}</span>)</legend>
