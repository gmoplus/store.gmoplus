...
<div id="nav_bar">
    ...
    {if $aRights.$cKey.add && !$smarty.get.action}
        <a href="{$rlBaseC}action=add" class="button_bar"><span class="left"></span><span class="center-add">{$lang.add_type}</span><span class="right"></span></a>
    {/if}
    {if $aRights.$cKey.edit && $smarty.get.action == 'build'}
        ...
        {if $smarty.get.form !== 'grid_form'}
            <a href="{$rlBase}index.php?controller=account_types&action=build&form=grid_form&key={$category_info.Key}"
               class="button_bar"
            >
                <span class="left"></span>
                <span class="center_build">{$lang.grid_form}</span>
