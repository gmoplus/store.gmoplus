        ...
        <div class="tab_area listing listing_details">
                    ...
                    {if $photos}
                        ...
                        {foreach from=$photos item='photo' name='photosF'}
                            <li class="media-{$photo.Type}">
                                {if $photo.Video_type == 'local'}
                                    <video><source src="{$photo.Thumbnail}" type="video/mp4"></source></video>
                                {else}
            ...
            {foreach from=$photos item='photo' name='photosF'}
                galleryData.push({$smarty.ldelim}
                    ...
                    src: '{if $photo.Type == 'video'}{if $photo.Original == 'youtube'}https://www.youtube.com/embed/{$photo.Photo}?rel=0{else}{$photo.Thumbnail}{/if}{else}{$photo.Photo}{/if}',
                    thumb: '{if $photo.Type == 'video' && $photo.Original != 'youtube'}{$rlTplBase}img/play.svg{else}{$photo.Thumbnail}{/if}',
                    caption: "{if $photo.Description}{$photo.Description}{else}{$pageInfo.name}{/if}",
                    youtubeKey: "{if $photo.Type == 'video' && $photo.Original == 'youtube'}{$photo.Photo}{/if}",
                {$smarty.rdelim});
            {/foreach}
            {literal}
