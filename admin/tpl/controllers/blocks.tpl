...
<div id="nav_bar">
    ...
    {if $aRights.$cKey.add && $smarty.get.action != 'add'}
        <a href="{$rlBaseC}action=add" class="button_bar"><span class="left"></span><span class="center-add">{$lang.add_block}</span><span class="right"></span></a>
    {/if}
...
{if $smarty.get.action}
     ...
     * @since 4.7.2
     *}
    {if $block.Key|strpos:'ltcategories_' === 0 || $block.Key|strpos:'ltsb_' === 0 || $block.Key === 'my_profile_sidebar' || $block.Key === 'more_news_block'}
    ...
    {include file='blocks'|cat:$smarty.const.RL_DS|cat:'m_block_start.tpl'}
        <form action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;block={$smarty.get.block}{/if}" method="post" enctype="multipart/form-data">
                                ...
                                <div id="pages_cont" {if !empty($sPost.show_on_all)}style="display: none;"{/if}>
                                    ...
                                    <tr>
                                        ...
                                        <div style="padding: 2px 8px;">
                                            <input data-key="{$page.Key}" data-controller="{$page.Controller}" class="checkbox" {if isset($bPages.$pId)}checked="checked"{/if} id="page_{$page.ID}" type="checkbox" name="pages[{$page.ID}]" value="{$page.ID}" /> <label class="cLabel" for="page_{$page.ID}">{$page.name}</label>
                                        </div>
            ...
            {if $block_options}
                {foreach from=$block_options item='option' key='option_key'}
                <tr id="box_option_{$option_key}"{if $main_group_type && $block.Key != $main_group_type && $option_key|in_array:$dynamic_box_options && $block_options.group_categories.default} class="hide"{/if}>
                    <td class="name">{phrase key='blocks+option+'|cat:$option_key}</td>
                    <td class="field">
                        {if $option.type == 'boolean'}
                            <label><input {if $option.default}checked="checked"{/if} type="radio" name="options[{$option_key}]" value="1" /> {$lang.yes}</label>
                            <label><input {if !$option.default}checked="checked"{/if} type="radio" name="options[{$option_key}]" value="0" /> {$lang.no}</label>
                        {elseif $option.type == 'select'}
                            <select name="options[{$option_key}]">
                                {foreach from=$option.values item='value'}
                                <option value="{$value}" {if $value == $option.default}selected="selected"{/if}>{phrase key='blocks+option_value+'|cat:$value}</option>
                                {/foreach}
                            </select>
                        {elseif $option.type == 'number'}
                            <input type="text" value="{$option.default}" name="options[{$option_key}]" class="numeric wauto"{if $option.values} size="{$option.values}"{/if} />
                        {/if}

                        {assign var='option_hint' value='blocks+option_hint+'|cat:$option_key}

                        {if $option_key == 'group_categories'}
                            {if $main_group_type && $block.Key != $main_group_type}
                                {assign var='edit_box_link' value=$rlBaseC|cat:'action=edit&block='|cat:$main_group_type}
                                {assign var='replace' value='<a target="_blank" class="static" href="'|cat:$edit_box_link|cat:'">$1</a>'}
                                <span class="field_description group_categories_hint{if !$block_options.group_categories.default} hide{/if}">
                                    {$lang.group_categories_hint|regex_replace:'/\[(.*)\]/':$replace}
                                </span>
                            {/if}
                        {elseif $lang.$option_hint}
                            <span class="field_description">{$lang.$option_hint}</span>
                        {/if}
                    </td>
                </tr>
                {/foreach}
        ...
        var group_option_ids = '#box_option_{',#box_option_'|implode:$dynamic_box_options}';
        {literal}
        $(function(){
